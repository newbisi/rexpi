...
def buerrest(n, w):
    ...
    # return estimate for given n and w
...
def buerrest_getw(n, tol):
    # error estimate for best unitary approximation
    # return w s.t. the approximation to exp(iwx) of degree (n,n) has an error < tol
    nfacx = np.sum(np.log(np.arange(n+1,2*n+1)))
    efaclog = -2*nfacx-np.log(2*n+1)
...
def buerrest_getn(w, tol):
    # error estimate for best unitary approximation
    # return n s.t. the approximation to exp(iwx) of degree (n,n) has an error < tol
